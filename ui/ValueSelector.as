package ui
{
   ...
   public class ValueSelector extends Sprite
   {
      ...
      public function pressEvent(param1:MouseEvent) : *
      {
         if(this._enabled)
         {
            this._sens = param1.currentTarget == this.bt_up?Number(Number(1)):Number(Number(-1));
            this._oldPos = mouseY;
      ...
      public function verifi(param1:Object) : Number
      {
         ...
         _loc3_ = Math.round(_loc3_ / this._resolution) * this._resolution;
         var _loc4_:Array = String(this._resolution).split(".");
         if(_loc4_.length > 1)
         {
            _loc2_ = _loc4_[1].length;
            _loc3_ = Math.round(_loc3_ * Math.pow(10,_loc2_)) / Math.pow(10,_loc2_);
         }
         else
         {
            _loc3_ = Math.round(_loc3_);
         }
         if(_loc3_ < this._minValue)
         {
            _loc3_ = this._minValue;
         }
         if(_loc3_ > this._maxValue)
         {
            _loc3_ = this._maxValue;
         }
         return _loc3_;
      }
      ...
      public function set enabled(param1:Boolean) : *
      {
         ...
         this.txt.backgroundColor = !!param1?uint(uint(16777215)):uint(uint(14540253));
      }
   }
}
