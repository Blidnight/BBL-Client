package fx
{
   ...
   public class UserFx extends Emotional
   {
      ...
      public function haveFoot() : Boolean
      {
         ...
         if(skin)
         {
            _loc1_ = skin.haveFoot;
         }
         if(this._overHaveFoot.v)
         {
            _loc1_ = _loc1_ && Boolean(this._overHaveFoot.v - 1);
         }
         return _loc1_;
      }
      ...
      public function executeFXMessage(param1:Object, param2:uint, param3:uint, param4:Boolean, param5:Boolean, param6:uint) : *
      {
         ...
         else if(param2 == 3)
         {
            if(param4)
            {
               _loc8_ = new SkinColor();
               _loc8_.readBinaryColor(param1);
         ...
         else if(param2 == 4)
         {
            ...
            if(param4)
            {
               ...
               {
                  _loc11_ = uint(param1.bitReadUnsignedInt(6));
                  if(_loc11_ == 2)
                  {
                  ...
                  {
                     _loc10_ = new UserFxOverloadEvent("onOverLoadData");
                     _loc10_.uol = _loc9_;
                     _loc10_.walker = this;
               ...
               if(_loc9_.newOne)
               {
                  _loc10_ = new UserFxOverloadEvent("onNewOverLoad");
                  _loc10_.uol = _loc9_;
                  _loc10_.walker = this;
            ...
            else
            {
               ...
               while(_loc7_ < this.overloadList.length)
               {
                  if(this.overloadList[_loc7_].fxId == param2 && this.overloadList[_loc7_].fxSid == param3)
                  {
                     ...
                     _loc10_ = new UserFxOverloadEvent("onRemoveOverLoad");
                     _loc10_.uol = this.overloadList[_loc7_];
                     _loc10_.walker = this;
                     this.dispatchEvent(_loc10_);
         ...
         {
            _loc13_ = new SkinAction();
            _loc13_.skinByte = param1.bitReadUnsignedInt(32);
            ...
            _loc14_ = param1.bitReadBoolean();
            if(_loc14_)
            {
               _loc13_.data = param1.bitReadBinaryData();
               _loc13_.data.bitPosition = 0;
            }
            if(skin && (_loc13_.endCause == 1 || skinByte == _loc13_.skinByte))
            {
         ...
         else if(param2 == 6)
         {
            if(param4)
            {
               ...
               if(param1.bitReadBoolean())
               {
                  _loc17_ = param1.bitReadBinaryData();
                  _loc17_.bitPosition = 0;
               }
      ...
      {
         return this._firstDirection == 0?Boolean(Boolean(super.direction)):Boolean(Boolean(this._overDirection.directionValue));
      }
      ...
      public function set floodPunished(param1:Boolean) : *
      {
         ...
         alpha = !!param1?Number(Number(0.5)):Number(Number(1));
         if(camera)
         {
      ...
      public function setFilter(param1:Object, param2:uint = 0, param3:Array = null) : *
      {
         ...
         if(param3 == this._skinFilter && skin)
         {
            skin.filters = param3;
         }
      }
      ...
      public function updateLightEffect() : *
      {
         if(skin)
         {
            ...
            else if(!this.lightEffect && this._lightFilter)
            {
      ...
      {
         return !!this._overSkinAction?uint(uint(this._overSkinAction.skinActionValue)):uint(uint(super.skinAction));
      }
      ...
      {
         return !!this._overSkinAction?uint(uint(this._overSkinAction.skinActionPriorityValue)):uint(uint(super.skinActionPriority));
      }
      ...
      public function updateOverloadCache() : *
      {
         ...
         this._overGravity = {"v":1};
         this._overHaveFoot = {"v":0};
         var _loc3_:Number = this._overHide;
         this._overHide = -2;
         var _loc4_:Number = this._overShift;
         this._overShift = -2;
         var _loc5_:Number = this._overWalk;
         this._overWalk = -2;
         var _loc6_:Number = this._overJump;
         this._overJump = -2;
         var _loc7_:Number = this._overSkinScale;
         this._overSkinScale = 1;
         var _loc8_:* = null;
         var _loc9_:UserFxOverloadItem = this._overSkinAction;
         this._overSkinAction = null;
         var _loc10_:SkinColor = this._overSkinColor;
         this._overSkinColor = null;
         var _loc11_:UserFxOverloadItem = this._overDirection;
         var _loc12_:Boolean = super.direction;
         this._overDirection = null;
         var _loc13_:int = -1;
         _subSkinOffset.y = 0;
         _subSkinOffset.x = 0;
         resetAcceleration();
         var _loc14_:int = 0;
         while(_loc14_ < this.overloadList.length)
         {
            if(this.overloadList[_loc14_].type == 0)
            {
               if(this.overloadList[_loc14_].walkSpeed)
               {
                  this._overWalkSpeed = {"v":this._overWalkSpeed.v + this.overloadList[_loc14_].walkSpeedValue};
               }
               if(this.overloadList[_loc14_].swimSpeed)
               {
                  this._overSwimSpeed = {"v":this._overSwimSpeed.v + this.overloadList[_loc14_].swimSpeedValue};
               }
               if(this.overloadList[_loc14_].jumpStrength)
               {
                  this._overJumpStrength = {"v":this._overJumpStrength.v + this.overloadList[_loc14_].jumpStrengthValue};
               }
               if(this.overloadList[_loc14_].gravity)
               {
                  this._overGravity = {"v":this._overGravity.v + this.overloadList[_loc14_].gravityValue};
               }
               if(this.overloadList[_loc14_].skinScale)
               {
                  this._overSkinScale = this._overSkinScale + this.overloadList[_loc14_].skinScaleValue;
               }
               if(this.overloadList[_loc14_].skinOffsetX)
               {
                  _subSkinOffset.x = _subSkinOffset.x + this.overloadList[_loc14_].skinOffsetXValue;
               }
               if(this.overloadList[_loc14_].skinOffsetY)
               {
                  _subSkinOffset.y = _subSkinOffset.y + this.overloadList[_loc14_].skinOffsetYValue;
               }
            }
            else if(this.overloadList[_loc14_].type == 1)
            {
               if(this.overloadList[_loc14_].walkSpeed)
               {
                  this._overWalkSpeed = {"v":this._overWalkSpeed.v * this.overloadList[_loc14_].walkSpeedValue};
            ...
            else if(this.overloadList[_loc14_].type == 2)
            {
               if(this.overloadList[_loc14_].walkSpeed)
               {
                  this._overWalkSpeed = {"v":this.overloadList[_loc14_].walkSpeedValue};
               }
               if(this.overloadList[_loc14_].swimSpeed)
               {
                  this._overSwimSpeed = {"v":this.overloadList[_loc14_].swimSpeedValue};
               }
               if(this.overloadList[_loc14_].jumpStrength)
               {
                  this._overJumpStrength = {"v":this.overloadList[_loc14_].jumpStrengthValue};
               }
               if(this.overloadList[_loc14_].gravity)
               {
                  this._overGravity = {"v":this.overloadList[_loc14_].gravityValue};
               }
               if(this.overloadList[_loc14_].skinScale)
               {
                  this._overSkinScale = this.overloadList[_loc14_].skinScaleValue;
               }
               if(this.overloadList[_loc14_].skinOffsetX)
               {
                  _subSkinOffset.x = this.overloadList[_loc14_].skinOffsetXValue;
               }
               if(this.overloadList[_loc14_].skinOffsetY)
               {
                  _subSkinOffset.y = this.overloadList[_loc14_].skinOffsetYValue;
               }
            }
            if(this.overloadList[_loc14_].hideSkin)
            {
               this._overHide = Number(this.overloadList[_loc14_].hideSkinValue);
            }
            if(this.overloadList[_loc14_].shift)
            {
               this._overShift = Number(this.overloadList[_loc14_].shiftValue);
            }
            if(this.overloadList[_loc14_].walk)
            {
               this._overWalk = this.overloadList[_loc14_].walkValue;
            }
            if(this.overloadList[_loc14_].jump)
            {
               this._overJump = this.overloadList[_loc14_].jumpValue;
            }
            if(this.overloadList[_loc14_].skinAction)
            {
               this._overSkinAction = this.overloadList[_loc14_];
            }
            if(this.overloadList[_loc14_].haveFoot)
            {
               this._overHaveFoot = {"v":(!!this.overloadList[_loc14_].haveFootValue?2:1)};
            }
            if(this.overloadList[_loc14_].floorNormalAccel)
            {
               floorNormalAccel = this.overloadList[_loc14_].floorNormalAccelValue;
            }
            if(this.overloadList[_loc14_].floorAccel)
            {
               floorAccel = this.overloadList[_loc14_].floorAccelValue;
            }
            if(this.overloadList[_loc14_].floorAccelFactor)
            {
               floorAccelFactor = this.overloadList[_loc14_].floorAccelFactorValue;
            }
            if(this.overloadList[_loc14_].floorDecel)
            {
               floorDecel = this.overloadList[_loc14_].floorDecelValue;
            }
            if(this.overloadList[_loc14_].floorDecelFactor)
            {
               floorDecelFactor = this.overloadList[_loc14_].floorDecelFactorValue;
            }
            if(this.overloadList[_loc14_].flyAccel)
            {
               flyAccel = this.overloadList[_loc14_].flyAccelValue;
            }
            if(this.overloadList[_loc14_].flyAccelFactor)
            {
               flyAccelFactor = this.overloadList[_loc14_].flyAccelFactorValue;
            }
            if(this.overloadList[_loc14_].flyDecel)
            {
               flyDecel = this.overloadList[_loc14_].flyDecelValue;
            }
            if(this.overloadList[_loc14_].flyDecelFactor)
            {
               flyDecelFactor = this.overloadList[_loc14_].flyDecelFactorValue;
            }
            if(this.overloadList[_loc14_].pseudo)
            {
               _loc8_ = this.overloadList[_loc14_];
            }
            if(this.overloadList[_loc14_].direction)
            {
               this._overDirection = this.overloadList[_loc14_];
            }
            if(this.overloadList[_loc14_].skinColor && this.overloadList[_loc14_].skinColorValue)
            {
               this._overSkinColor = this.overloadList[_loc14_].skinColorValue;
            }
            if(this.overloadList[_loc14_].skinId)
            {
               if(this._firstSkinId.v < 0)
               {
                  this._firstSkinId = {"v":skinId};
               }
               _loc13_ = this.overloadList[_loc14_].skinIdValue;
            }
            _loc14_++;
         }
         ...
         if(skin)
         {
            if(_loc3_ == -2 && this._overHide != -2)
            {
               skin.visible = this._overHide == 0;
            }
            else if(_loc3_ != -2 && this._overHide == -2)
            {
               skin.visible = true;
            }
            else if(_loc3_ != this._overHide)
            {
               skin.visible = this._overHide == 0;
            }
         }
         if(_loc4_ == -2 && this._overShift != -2)
         {
            super.shiftKey = this._overShift == 1;
         }
         else if(_loc4_ != -2 && this._overShift == -2)
         {
            super.shiftKey = this._firstShift;
         }
         else if(_loc4_ != this._overShift)
         {
            super.shiftKey = this._overShift == 1;
         }
         if(_loc5_ == -2 && this._overWalk != -2)
         {
            super.walk = this._overWalk;
         }
         else if(_loc5_ != -2 && this._overWalk == -2)
         {
            super.walk = this._firstWalk;
         }
         else if(_loc5_ != this._overWalk)
         {
            super.walk = this._overWalk;
         }
         if(_loc6_ == -2 && this._overJump != -2)
         {
            super.jump = this._overJump;
         }
         else if(_loc6_ != -2 && this._overJump == -2)
         {
            super.jump = this._firstJump;
         }
         else if(_loc6_ != this._overJump)
         {
            super.jump = this._overJump;
         }
         if(_loc7_ != this._overSkinScale)
         {
            skinScale = this._overSkinScale;
         }
         if(_loc8_ && !this.overPseudo)
         {
            this.overPseudo = _loc8_.duplicate();
         }
         else if(!_loc8_ && this.overPseudo)
         {
            this.overPseudo = null;
         }
         else if(_loc8_ && this.overPseudo)
         {
            if(_loc8_.pseudoValue != this.overPseudo.pseudoValue || _loc8_.type != this.overPseudo.type)
            {
               this.overPseudo = _loc8_.duplicate();
            }
         }
         if(this._overSkinAction && !_loc9_)
         {
         ...
         else if(!this._overSkinAction && _loc9_)
         {
            super.skinAction = this._firstSkinAction;
            super.skinActionPriority = this._firstSkinActionPriority;
            this._overSkinAction = null;
         }
         else if(_loc9_ != this._overSkinAction)
         {
            super.skinAction = this._overSkinAction.skinActionValue;
            super.skinActionPriority = this._overSkinAction.skinActionPriorityValue;
         }
         if(this._overSkinColor && !_loc10_)
         {
            this._firstSkinColor = super.skinColor;
            updateSkinColor();
         }
         else if(!this._overSkinColor && _loc10_)
         {
            _loc1_ = this._firstSkinColor;
            this._firstSkinColor = null;
            this.skinColor = _loc1_;
         }
         else if(_loc10_ != this._overSkinColor)
         {
            updateSkinColor();
         }
         if(this._overDirection && !_loc11_)
         {
            this._firstDirection = Number(super.direction) * 2 - 1;
            super.direction = this._overDirection.directionValue;
         }
         else if(!this._overDirection && _loc11_)
         {
            _loc2_ = this._firstDirection == 1;
            this._firstDirection = 0;
         ...
         if(_loc13_ < 0 && this._firstSkinId.v >= 0)
         {
      ...
      public function checkForUnlight() : *
      {
         ...
         else if(!this._lightAdded && this._lightEffect.v)
         {
            this.addUnLight();
         }
      }
