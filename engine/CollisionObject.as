package engine
{
   public class CollisionObject
   {
      ...
      private function _calculateNormal() : void
      {
         ...
         var _loc2_:* = null;
         var _loc3_:* = 0;
         var _loc4_:Object = _pxArea[1 + this.lastPixel.x - this.colPixel.x + "_" + (1 + this.lastPixel.y - this.colPixel.y)];
         var _loc5_:Object = _loc4_.next;
         var _loc6_:* = 0;
         while(_loc5_ != _loc4_ && _loc6_ < 5)
         {
            _loc3_ = uint(this.collisionBody.map.getPixel(this.colPixel.x + _loc5_.x - this.collisionBody.position.x,this.colPixel.y + _loc5_.y - this.collisionBody.position.y));
            if(_loc3_ != 0 && !this.exclude[String(_loc3_)])
            {
               _loc1_ = _loc5_;
               break;
            }
         ...
         _loc6_ = 0;
         while(_loc5_ != _loc4_ && _loc6_ < 5 && _loc7_ < 5)
         {
            _loc3_ = uint(this.collisionBody.map.getPixel(this.colPixel.x + _loc5_.x - this.collisionBody.position.x,this.colPixel.y + _loc5_.y - this.collisionBody.position.y));
            if(_loc3_ != 0 && !this.exclude[String(_loc3_)])
            {
         ...
         if(_loc1_ && _loc2_ && _loc1_ != _loc2_)
         {
