   ...
   public class Scroller
   {
      ...
      public function scrollX(param1:Number) : *
      {
         ...
         if(this.relativeObject)
         {
            ...
            while(_loc4_ < this.itemList.length)
            {
               ...
               _loc6_ = !!this.depthScrollEffect?Number(Number(1 / (_loc5_.plan / 5 / 10 + 0.9))):Number(Number(1));
               _loc7_ = 0;
      ...
      public function step() : *
      {
         ...
         if(this.activTarget && (this.scrollMode == 0 || this.scrollMode == 1))
         {
            ...
            if(_loc2_ > 0)
            {
               this.curSpd = this.curSpd + Math.max(_loc2_ / 100,0.9);
            }
            _loc3_ = this.curSpd;
            if(_loc2_ < 200)
            {
               _loc3_ = _loc3_ * Math.min(0.1 + 2 * (_loc2_ - 10) / 190,1);
            }
      ...
      public function stepScrollTo(param1:Number, param2:Number, param3:Object = null) : *
      {
         ...
         {
            _loc4_ = 0;
            _loc5_ = this.mapWidth - this.screenWidth;
            if(_loc5_ > 0)
            {
               _loc10_ = (param1 - this.screenWidth / 2) / _loc5_;
               _loc4_ = Number(Math.max(Math.min(_loc10_,1),0));
            }
            _loc6_ = Math.abs(this.itemList[0].target.x + param1 - this.screenWidth / 2) >= this.screenWidth * this.crantMarge;
            _loc6_ = _loc6_ && _loc4_ != Math.round(this.curScrollX * 100) / 100;
            _loc7_ = 0;
            _loc8_ = this.mapHeight - this.screenHeight;
            if(_loc8_ > 0)
            {
               _loc11_ = (param2 - this.screenHeight / 2) / _loc8_;
               _loc7_ = Number(Math.max(Math.min(_loc11_,1),0));
            }
            _loc9_ = Math.abs(this.itemList[0].target.y + param2 - this.screenHeight / 2) >= this.screenHeight * this.crantMarge;
            _loc9_ = _loc9_ && _loc7_ != Math.round(this.curScrollY * 100) / 100;
