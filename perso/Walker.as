package perso
{
   ...
   public class Walker extends SkinManager
   {
      ...
      {
         var _loc1_:* = 0;
         var _loc2_:uint = Math.round(this.position.x + this.skinOffset.x + this._subSkinOffset.x);
         var _loc3_:uint = Math.round(this.position.y + this.skinOffset.y + this._subSkinOffset.y);
         if(_loc2_ != x)
         {
            x = _loc2_;
         }
         ...
         if(!this.paused)
         {
            ...
            {
               _loc1_ = uint(this.walking != 0 || this.jumping != 0?uint(uint(21)):uint(uint(20)));
            }
            ...
            {
               _loc1_ = uint(this.speed.y > 0?uint(uint(11)):uint(uint(10)));
            }
            if(this.skinAction || this.skinActionPriority)
            {
               if((this.skinActionPriority & 1) == 1 && _loc1_ == 0)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 2) == 2 && _loc1_ == 1)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 4) == 4 && _loc1_ == 10)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 8) == 8 && _loc1_ == 11)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 16) == 16 && _loc1_ == 20)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 32) == 32 && _loc1_ == 21)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 64) == 64 && _loc1_ == 30)
               {
                  _loc1_ = uint(this.skinAction);
               }
               else if((this.skinActionPriority & 128) == 128 && _loc1_ == 50)
               {
                  _loc1_ = uint(this.skinAction);
               }
            }
      ...
      override public function step() : *
      {
         ...
         if(this.activity && !this.paused)
         {
            ...
            while(this.processList.length && _loc1_ < 5)
            {
               ...
               if(_loc2_ == 1 && this._onFloor.v)
               {
                  this.floorProcess();
                  this._floorProcess = true;
               }
               if(_loc2_ == 2 && this._underWater.v)
               {
                  this.waterProcess();
               }
               if(_loc2_ == 3 && this._grimpe.v && this._accroche.v)
               {
                  this.grimpeProcess();
               }
      ...
      function onGrimpeTimeOut(param1:Event = null) : *
      {
         this.accroche = false;
         this.speed.x = 0;
         this.speed.y = 0;
      ...
      function grimpeProcess() : *
      {
         ...
         var _loc7_:WalkerPhysicEvent = WalkerPhysicEvent.getDefaultEvent(this,"floorEvent");
         var _loc8_:Object = this._physic.getCollisionPixelData(Math.floor(this.position.x),Math.floor(this.position.y - this.skinPhysicHeight));
         if(_loc8_.pxl && !this.physic.cloudTile[_loc8_.pxl])
         {
            this.accroche = false;
            this.speed.x = 0;
            this.speed.y = 0;
         ...
         {
            _loc1_ = 2;
            this.speed.x = this.walking * this.walkSpeed / _loc1_;
            this.speed.y = this.jumping * this.walkSpeed / -_loc1_;
            _loc2_ = this.calculateColliWith(this.position,this.speed,0,this.speed.y <= 0?this.physic.cloudTile:null);
            _loc3_ = this.calculateColliWith(this.position,this.speed,this.skinPhysicHeight,this.physic.cloudTile);
            _loc4_ = this.getFirstCol([_loc2_,_loc3_]);
            if(_loc4_)
            {
               this.position.x = _loc4_.lastPixel.x + 0.5;
               this.position.y = _loc4_.lastPixel.y + 0.5 + (this.position.y - _loc4_.originalSegment.ptA.y);
               _loc5_ = true;
               if(_loc4_ == _loc2_)
               {
                  _loc2_.calculateNormal();
                  _loc6_ = Math.abs(Math.atan2(_loc2_.normal.x,_loc2_.normal.y) * 180 / Math.PI);
                  if(_loc6_ >= 120 && this.jumping < 1)
                  {
                     this.speed.x = 0;
                     this.speed.y = 0;
                     this._floorColor = {"v":_loc4_.color};
                     this.onFloorEngine = true;
                     this.surfaceBody = _loc2_.collisionBody;
                     _loc5_ = false;
                     _loc7_.lastColor = 0;
                     _loc7_.eventType = 10;
                     _loc7_.colData = _loc4_;
                     _loc7_.newColor = _loc4_.color;
      ...
      function flyProcess(param1:PhysicBody = null) : *
      {
         var _loc2_:* = null;
         var _loc3_:* = null;
         var _loc4_:* = null;
         ...
         else
         {
            ...
            if(this.speed.y < 0)
            {
               _loc7_ = this._physic.getCollisionPixelData(Math.floor(this.position.x),Math.floor(this.position.y - this.skinPhysicHeight));
               if(_loc7_.pxl)
               {
            ...
            if(param1)
            {
               ...
               _loc9_.x = _loc9_.x + _loc8_.x;
               _loc9_.y = _loc9_.y + _loc8_.y;
               _loc8_.x = _loc8_.x / -this.stepRate;
               _loc8_.y = _loc8_.y / -this.stepRate;
            ...
            else
            {
               ...
               if(_loc10_ == 1 && this.speed.x <= this.walkSpeed || _loc10_ == -1 && this.speed.x >= -this.walkSpeed)
               {
                  if(this.flyAccel)
                  {
                     this.speed.x = this.speed.x + _loc10_ * this.flyAccelFactor * this.stepRate;
                  }
                  else
                  {
            ...
            _loc5_ = this.getFirstCol([_loc3_,_loc4_]);
            if(_loc5_ && _loc5_ == _loc4_)
            {
               ...
               _loc6_ = Math.abs(Math.atan2(_loc5_.normal.x,_loc5_.normal.y) * 180 / Math.PI);
               if(_loc6_ <= 90)
               {
                  ...
                  if(param1)
                  {
                     this.speed.x = this.speed.x + param1.speed.x;
                     this.speed.y = this.speed.y + param1.speed.y;
                  }
            ...
            if(_loc5_ && _loc5_ == _loc3_)
            {
               ...
               else
               {
                  ...
                  this.speed.y = this.speed.y > 0?Number(Number(_loc3_.normal.y * this.walkSpeed)):Number(Number(0));
               }
               _loc2_.newSpeed = this.speed.duplicate();
               this.dispatchEvent(_loc2_);
            }
      ...
      function waterProcess() : *
      {
         ...
         if(_loc3_ == 1)
         {
            _loc1_ = this._physic.getCollisionPixelData(Math.floor(this.position.x),Math.floor(this.position.y - this.skinPhysicHeight));
            if(_loc1_.pxl && !this.physic.cloudTile[_loc1_.pxl])
            {
               _loc3_ = 0;
               if(this.speed.y < 0)
               {
         ...
         if(this.walking == 1 && this.speed.x < this.swimSpeed / 1.7 || this.walking == -1 && this.speed.x > -this.swimSpeed / 1.7)
         {
            this.speed.x = this.speed.x + this.stepRate * this.walking * this.swimSpeed / 820;
         }
         if(_loc3_ == 1 && this.speed.y < this.swimSpeed / 1.7 || _loc3_ == -1 && this.speed.y > -this.swimSpeed / 1.7)
         {
            this.speed.y = this.speed.y - this.stepRate * _loc3_ * this.swimSpeed / 820;
         }
         ...
         var _loc4_:CollisionObject = this.calculateColliWith(this.position,this.speed,0,this.speed.y < 0?this.physic.cloudTile:null);
         var _loc5_:CollisionObject = this.calculateColliWith(this.position,this.speed,Math.round(this.skinPhysicHeight),this.physic.cloudTile);
         var _loc6_:CollisionObject = this.getFirstCol([_loc4_,_loc5_]);
         if(_loc6_)
         {
            this.position.x = _loc6_.lastPixel.x + 0.5;
            this.position.y = _loc6_.lastPixel.y + 0.5 + (_loc6_ == _loc5_?Math.round(this.skinPhysicHeight):0);
            if(this.walking != 0 || _loc3_ != 0)
            {
               _loc6_.calculateNormal();
               this.speed.x = this.speed.x + _loc6_.normal.x / 12;
               this.speed.y = _loc6_.normal.y / 12;
            }
            else
            {
               this.speed.x = 0;
      ...
      function floorProcess() : *
      {
         ...
         else if(this.floorSlide != 0 || this.walkSpeed * this.walking != 0 || this.speed.x != 0 && (this.floorAccel || this.floorDecel))
         {
            ...
            _loc1_.eventType = 0;
            _loc1_.lastColor = this._floorColor.v;
            _loc2_ = this.walkSpeed + 0.044;
            if(this.walking == 1 && this.speed.x <= _loc2_ || this.walking == -1 && this.speed.x >= -_loc2_)
            {
               if(this.floorAccel)
               {
               ...
               {
                  this.speed.x = _loc2_ * this.walking;
               }
               if(this.walking == 1 && this.speed.x >= _loc2_ || this.walking == -1 && this.speed.x <= -_loc2_)
               {
                  this.speed.x = this.walking * _loc2_;
               }
            }
            ...
            {
               _loc9_ = new DDpoint();
               _loc9_.y = this.position.y;
               _loc9_.x = this.position.x - 3;
               _loc10_ = this.getVpixel(_loc9_,4,4);
               _loc9_.x = this.position.x + 3;
               _loc11_ = this.getVpixel(_loc9_,4,4);
               _loc12_ = _loc11_.ypos - _loc10_.ypos;
               this.speed.x = this.speed.x + this.floorNormalAccel * this.gravity * this.stepRate * _loc12_ / 10;
            }
            _loc3_ = Math.abs((this.speed.x + this.floorSlide) * this.stepRate);
            _loc3_ = _loc3_ + -this._walkSubStep;
            _loc4_ = 0;
            _loc5_ = new DDpoint();
            _loc6_ = this._floorColor.v;
            _loc5_.init();
            _loc5_.x = this.position.x;
            _loc5_.y = this.position.y;
            while(_loc3_ > _loc4_)
            {
               _loc5_.x = _loc5_.x + ((this.speed.x + this.floorSlide) * this.stepRate > 0?1:-1);
               if(_loc5_.x < 0 || _loc5_.x >= this.physic.environmentBodyList[0].map.width)
               {
                  this.speed.y = (_loc5_.y - this.position.y) / this.stepRate;
                  break;
               }
               _loc13_ = this.getVpixel(_loc5_,4,this.gravity * this.stepRate * 2 + 2);
               if(_loc13_.pxl == null)
               {
                  if(_loc13_.dir == 1)
                  {
               ...
               {
                  _loc6_ = _loc13_.pxl;
                  _loc5_.y = Math.floor(_loc13_.ypos) + 0.5;
                  this.surfaceBody = _loc13_.body;
                  _loc4_ = Number(_loc5_.getPointDistance(this.position));
               }
            }
            this._walkSubStep = _loc4_ - _loc3_;
            if(_loc14_)
            {
               this.speed.y = (_loc5_.y - this.position.y) / this.stepRate;
               if(_loc6_ != this._floorColor.v)
               {
                  this._floorColor = {"v":_loc6_};
                  _loc1_.eventType = 30;
                  _loc1_.newColor = _loc6_;
               }
               else
               {
                  _loc1_ = null;
            ...
            _loc7_ = new Segment();
            _loc7_.init();
            _loc7_.ptA.x = this.position.x;
            _loc7_.ptA.y = this.position.y - this.skinPhysicHeight;
            _loc7_.ptB.x = _loc5_.x;
            _loc7_.ptB.y = _loc5_.y - this.skinPhysicHeight;
            _loc7_.lineCoef();
            _loc8_ = this.physic.getSurfaceCollision(_loc7_,this.physic.cloudTile);
            if(_loc8_)
            {
               this.speed.x = 0;
               this.speed.y = 0;
            ...
            {
               this.position.x = _loc5_.x;
               this.position.y = _loc5_.y;
            }
      ...
      public function onCollisionBodyMove(param1:PhysicBodyEvent) : *
      {
         ...
         {
            this.position.x = this.position.x + (param1.body.position.x - param1.body.lastPosition.x);
            this.position.y = this.position.y + (param1.body.position.y - param1.body.lastPosition.y);
      ...
      private function getVpixel(param1:DDpoint, param2:Number, param3:Number) : Object
      {
         var _loc4_:int = 0;
         var _loc5_:* = null;
         var _loc6_:DDpoint = new DDpoint();
         _loc6_.x = param1.x;
         _loc6_.y = param1.y;
         var _loc7_:Object = this.physic.getCollisionPixelData(_loc6_.x,_loc6_.y);
         if(_loc7_.pxl != 0)
         {
            _loc4_ = 0;
            while(_loc4_ < param2)
            {
               _loc6_.y--;
               _loc5_ = _loc7_;
               _loc7_ = this.physic.getCollisionPixelData(_loc6_.x,_loc6_.y);
               if(_loc7_.pxl == 0)
               {
                  return {
                     "pxl":_loc5_.pxl,
      ...
      function getFirstCol(param1:Array) : CollisionObject
      {
         var _loc2_:* = undefined;
         ...
         while(_loc6_ < param1.length)
         {
            if(param1[_loc6_])
            {
               _loc2_ = param1[_loc6_].lastPixel.duplicate();
               _loc2_.x = _loc2_.x + 0.5;
               _loc2_.y = _loc2_.y + 0.5;
               _loc3_ = _loc2_.getPointDistance(param1[_loc6_].originalSegment.ptA);
               if(_loc3_ < _loc5_ || _loc5_ < 0)
               {
      ...
      function calculateColliWith(param1:DDpoint, param2:DDpoint, param3:Number = 0, param4:Object = null, param5:PhysicBody = null) : CollisionObject
      {
         var _loc6_:* = new Segment();
         _loc6_.init();
      ...
      public function set underWater(param1:Boolean) : void
      {
         ...
         if(param1 != this._underWater.v)
         {
            ...
            else
            {
               ...
               _loc4_ = this.speed.duplicate();
               _loc4_.normalize();
               _loc5_ = Math.min(1 - Math.cos(Math.atan2(this.speed.x,this.speed.y)),1);
               _loc5_ = this.walking != 0 || this.jumping != 0?Number(Number(_loc5_)):Number(Number(0));
      ...
      public function set onFloor(param1:Boolean) : void
      {
         ...
         if(param1 != this._onFloor.v)
         {
            ...
            if(!this._processing)
            {
               _loc2_ = WalkerPhysicEvent.getDefaultEvent(this,"floorEvent");
               _loc2_.newColor = 0;
               _loc2_.eventType = !!param1?Number(Number(10)):Number(Number(0));
               _loc2_.lastColor = this._floorColor.v;
               this._floorColor = {"v":0};
               _loc2_.newSpeed = this.speed.duplicate();
               this.dispatchEvent(_loc2_);
      ...
      public function set physic(param1:Physic) : void
      {
         ...
         if(this._physic)
         {
            ...
            {
               _loc4_ = this._physic.getCollisionPixelData(Math.floor(this.position.x),Math.floor(this.position.y - _loc3_));
               if(!_loc4_.pxl)
               {
            ...
            {
               _loc4_ = this._physic.getCollisionPixelData(Math.floor(this.position.x + _loc3_),Math.floor(this.position.y));
               if(!_loc4_.pxl)
               {
            ...
            if(this.onFloor)
            {
               ...
               while(_loc3_ < 5)
               {
                  _loc4_ = this._physic.getCollisionPixelData(Math.round(this.position.x),Math.round(this.position.y + _loc3_ + 1));
                  if(_loc4_.pxl)
                  {
      ...
      public function get skinPhysicHeight() : Number
      {
         ...
         if(this._physic)
         {
            _loc1_ = Number(!!this.underWater?Number(Number(this._physic.skinWaterPhysicHeight)):Number(Number(this._physic.skinPhysicHeight)));
         }
         if(skin)
         {
            _loc1_ = Number(!!this.underWater?Number(Number(skin.skinWaterPhysicHeight)):Number(Number(skin.skinPhysicHeight)));
         }
         _loc1_ = Number(_loc1_ * skinScale);
         return _loc1_;
      }
      ...
      public function get skinGraphicHeight() : Number
      {
         ...
         if(this._physic)
         {
            _loc1_ = Number(!!this.underWater?Number(Number(this._physic.skinWaterGraphicHeight)):Number(Number(this._physic.skinGraphicHeight)));
         }
         if(skin)
         {
            _loc1_ = Number(!!this.underWater?Number(Number(skin.skinWaterGraphicHeight)):Number(Number(skin.skinGraphicHeight)));
         }
         _loc1_ = Number(_loc1_ * skinScale);
         return _loc1_;
      }
      ...
      public function get gravity() : Number
      {
         ...
         if(this._physic)
         {
            _loc1_ = Number(this._physic.gravity);
         }
         if(skin)
         {
            _loc1_ = Number(_loc1_ * skin.gravity);
         }
         _loc1_ = Number(_loc1_ * this._gravity);
         return _loc1_;
      }
      ...
      public function get walkSpeed() : Number
      {
         ...
         if(this._physic)
         {
            _loc1_ = Number(this._physic.walkSpeed);
         }
         _loc1_ = Number(_loc1_ * this._walkSpeed.v);
         if(this.shiftKey)
         {
         ...
         {
            _loc1_ = Number(_loc1_ * skin.walkSpeed);
         }
         return _loc1_;
      }
      ...
      public function get swimSpeed() : Number
      {
         ...
         if(this._physic)
         {
            _loc1_ = Number(this._physic.swimSpeed);
         }
         _loc1_ = Number(_loc1_ * this._swimSpeed.v);
         if(this.shiftKey)
         {
            _loc1_ = Number(_loc1_ * 0.5);
         }
         ...
         {
            _loc1_ = Number(_loc1_ * skin.swimSpeed);
         }
         return _loc1_;
      }
      ...
      public function get jumpStrength() : Number
      {
         ...
         if(this._physic)
         {
            _loc1_ = Number(this._physic.jumpStrength);
         }
         _loc1_ = Number(_loc1_ * this._jumpStrength.v);
         if(this.shiftKey)
         {
            _loc1_ = Number(_loc1_ * 0.8);
         }
         else if(skin)
         {
            _loc1_ = Number(_loc1_ * skin.jumpStrength);
         }
         return _loc1_;
      }
   }
