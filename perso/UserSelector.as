package perso
{
   ...
   public class UserSelector extends MovieClip
   {
      ...
      public function rebuildFriendList(param1:Event = null) : *
      {
         ...
         var _loc3_:Array = GlobalProperties.mainApplication.blablaland.friendList;
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_.length)
         {
            _loc3_[_loc4_].addEventListener("onStateChanged",this.friendStateChanged,false,0,true);
            if(_loc3_[_loc4_].connected && this._showOnlineFriendList || !_loc3_[_loc4_].connected && this._showOfflineFriendList)
            {
               _loc2_ = this.userList.node.addChild();
               _loc2_.data.FRIEND = _loc3_[_loc4_];
               _loc2_.data.TYPE = 2;
               _loc2_.data.PSEUDO = _loc3_[_loc4_].pseudo;
               _loc2_.data.UID = _loc3_[_loc4_].uid;
               _loc2_.icon = !!_loc3_[_loc4_].connected?"onLine":"offLine";
               _loc2_.text = _loc2_.data.PSEUDO;
            }
            _loc4_++;
         }
         this.userList.redraw();
