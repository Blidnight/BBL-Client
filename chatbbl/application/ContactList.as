   ...
   public class ContactList extends MovieClip
   {
      ...
      public function init(param1:Event) : *
      {
         if(stage)
         {
            ...
            parent.height = !!GlobalProperties.sharedObject.data.POPUP.POP_CONTACTLIST_H?Number(Number(GlobalProperties.sharedObject.data.POPUP.POP_CONTACTLIST_H)):Number(Number(180));
            parent.x = !!GlobalProperties.sharedObject.data.POPUP.POP_CONTACTLIST_X?Number(Number(GlobalProperties.sharedObject.data.POPUP.POP_CONTACTLIST_X)):Number(Number(parent.x));
            parent.y = !!GlobalProperties.sharedObject.data.POPUP.POP_CONTACTLIST_Y?Number(Number(GlobalProperties.sharedObject.data.POPUP.POP_CONTACTLIST_Y)):Number(Number(parent.y));
            parent.width = 100;
      ...
      public function rebuildBlackList() : *
      {
         ...
         var _loc2_:Array = Chat(GlobalChatProperties.chat).blablaland.blackList;
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_.length)
         {
            _loc1_ = this.blackListNode.addChild();
            _loc1_.data.CONTACT = _loc2_[_loc3_];
            _loc1_.text = _loc1_.data.CONTACT.pseudo;
            _loc3_++;
         }
      ...
      public function rebuildMuteList() : *
      {
         ...
         var _loc2_:Array = Chat(GlobalChatProperties.chat).blablaland.muteList;
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_.length)
         {
            _loc1_ = this.muteNode.addChild();
            _loc1_.data.CONTACT = _loc2_[_loc3_];
            _loc1_.text = _loc1_.data.CONTACT.pseudo;
            _loc3_++;
         }
      ...
      public function rebuildMapList() : *
      {
         ...
         var _loc2_:Array = Chat(GlobalChatProperties.chat).camera.userList;
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_.length)
         {
            if(!User(_loc2_[_loc3_]).clientControled)
            {
               _loc1_ = this.mapNode.addChild();
               _loc1_.data.USER = _loc2_[_loc3_];
               _loc1_.icon = "onLine";
               _loc1_.text = _loc1_.data.USER.pseudo;
            }
            _loc3_++;
         }
         this.reorderMapList();
      ...
      public function rebuildFriendList() : *
      {
         ...
         var _loc2_:Array = Chat(GlobalChatProperties.chat).blablaland.friendList;
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_.length)
         {
            _loc1_ = this.friendNode.addChild();
            _loc1_.data.FRIEND = _loc2_[_loc3_];
            _loc1_.data.FRIEND.addEventListener("onStateChanged",this.onFriendStateChanged,false,0,true);
            _loc1_.icon = !!_loc1_.data.FRIEND.connected?"onLine":"offLine";
            _loc1_.text = _loc1_.data.FRIEND.pseudo;
            _loc3_++;
         }
         this.reorderFriendList();
      }
      ...
      public function reorderFriendList() : *
      {
         ...
         {
            if(param1.data.FRIEND.connected && !param2.data.FRIEND.connected)
            {
               return -1;
            }
            if(!param1.data.FRIEND.connected && param2.data.FRIEND.connected)
