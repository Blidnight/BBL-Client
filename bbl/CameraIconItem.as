package bbl
{
   ...
   public class CameraIconItem extends Sprite
   {
      ...
      public function updateBulle() : *
      {
         ...
         else if(this.isOver && this._overBulle)
         {
            this.haveChrono = false;
      ...
      public function getChronoString() : String
      {
         ...
         if(this.chronoClip)
         {
            _loc1_ = Math.max(Math.round((this.chronoTo - GlobalProperties.serverTime) / 1000),0);
            if(_loc1_ > 59)
            {
               _loc2_ = Math.floor(_loc1_ / 60);
               _loc3_ = _loc1_ - _loc2_ * 60;
               _loc4_ = _loc4_ + (_loc2_.toString() + " min et ");
               _loc4_ = _loc4_ + (_loc3_.toString() + " sec");
            }
            else if(_loc1_ > 1)
            {
               _loc4_ = _loc4_ + (_loc1_.toString() + " secondes");
            }
            else
            {
               _loc4_ = _loc4_ + (_loc1_.toString() + " seconde");
            }
         }
         return _loc4_;
      }
      ...
      public function setChrono(param1:Boolean, param2:Number, param3:Number) : *
      {
         ...
         else if(!param1 && this.chronoClip)
         {
      ...
      public function set clickable(param1:Boolean) : *
      {
         var _loc2_:* = null;
         var _loc3_:int = 0;
         while(_loc3_ < this.frontContent.numChildren)
         {
            _loc2_ = MovieClip(this.frontContent.getChildAt(_loc3_));
            if(_loc2_.totalFrames == 2 && _loc2_ != this.chronoClip)
            {
               _loc2_.gotoAndStop(!!param1?"CLICKABLE":"UNCLICKABLE");
               break;
            }
            _loc3_++;
         }
