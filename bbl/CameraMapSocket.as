package bbl
{
   ...
   public class CameraMapSocket extends CameraInterface
   {
      ...
      override public function set userInterface(param1:InterfaceSmiley) : *
      {
         ...
         if(userInterface != param1 && param1)
         {
            _loc2_ = true;
         }
      ...
      public function flushAction(param1:Event = null) : *
      {
         ...
         var _loc4_:* = null;
         var _loc5_:int = 0;
         while(_loc5_ < this.actionList.length)
         {
            _loc3_ = this.actionList[_loc5_];
            if(_loc3_[2].currentTimer >= _loc3_[1] + this.latence)
            {
               if(_loc3_[0] == 2)
               {
                  _loc2_ = uint(_loc3_[3].bitReadUnsignedInt(GlobalProperties.BIT_METHODE_ID));
                  removeUser(_loc3_[2],_loc2_);
               }
               else if(_loc3_[0] == 3)
               {
                  _loc3_[2].readStateFromMessage(_loc3_[3]);
               }
               else if(_loc3_[0] == 4)
               {
                  _loc3_[2].readStateFromMessage(_loc3_[3]);
                  _loc4_ = WalkerPhysicEvent.getEventFromMessage(_loc3_[3]);
                  _loc4_.walker = _loc3_[2];
                  _loc4_.certified = true;
                  _loc3_[2].dispatchEvent(_loc4_);
               }
               else if(_loc3_[0] == 6)
               {
                  _loc3_[2].readFXChange(_loc3_[3]);
               }
               else if(_loc3_[0] == 9)
               {
                  _loc3_[2].addEventListener("endFx",this.onEndLeaveMapTeleportSameMap,false,0,true);
                  _loc3_[2].lastSocketMessage = _loc3_[3];
                  _loc2_ = uint(_loc3_[3].bitReadUnsignedInt(GlobalProperties.BIT_METHODE_ID));
                  _loc3_[2].leaveMapFX(_loc2_);
               }
               else if(_loc3_[0] == 1001)
               {
                  _loc2_ = uint(_loc3_[3].bitReadUnsignedInt(GlobalProperties.BIT_METHODE_ID));
                  removeUserFromTemp(_loc3_[2]);
                  addUser(_loc3_[2],_loc2_);
               }
               this.actionList.splice(_loc5_,1);
               _loc5_--;
            }
            _loc5_++;
         }
      ...
      public function changeMapStatus(param1:ServerMap, param2:uint, param3:uint) : *
      {
         ...
         else if(currentMap && param2 != 3)
         {
            currentMap.onChangeMapError(param2);
         }
         if(param2 == 17 && userInterface)
         {
      ...
      public function parsedMessage(param1:uint, param2:uint, param3:SocketMessageEvent) : *
      {
         ...
         if(param1 == 4)
         {
            if(param2 == 1)
            {
               ...
               _loc4_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_ERROR_ID));
               _loc5_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_METHODE_ID));
               if(!_loc4_)
               {
                  ...
                  {
                     _loc7_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_ID));
                     _loc8_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_PID));
                     if(this.blablaland.pid == _loc8_)
                     {
                        this.blablaland.pseudo = param3.message.bitReadString();
                        _loc9_ = uint(param3.message.bitReadUnsignedInt(3));
            ...
            {
               _loc11_ = new ServerMap();
               _loc11_.id = param3.message.bitReadUnsignedInt(GlobalProperties.BIT_MAP_ID);
         ...
         else if(param1 == 5)
         {
            ...
            {
               _loc15_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_ID));
               _loc16_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_PID));
               _loc12_ = new User();
               _loc12_.userId = _loc15_;
               _loc12_.userPid = _loc16_;
               _loc12_.pseudo = param3.message.bitReadString();
               _loc12_.sex = param3.message.bitReadUnsignedInt(3);
               _loc13_ = uint(param3.message.bitReadUnsignedInt(32));
            ...
            {
               _loc12_ = getUserByPid(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_PID));
               if(_loc12_ && _loc12_ != mainUser)
               {
            ...
            {
               _loc14_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_PID));
               _loc12_ = null;
               if(mainUser)
               {
            ...
            else if(param2 == 7)
            {
               ...
               _loc14_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_PID));
               _loc20_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_ID));
               _loc21_ = uint(param3.message.bitReadUnsignedInt(3));
               _loc22_ = param3.message.bitReadString();
               _loc23_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_SERVER_ID));
               _loc24_ = param3.message.bitReadString();
               param1 = param3.message.bitReadUnsignedInt(3);
               if(!this.blablaland.isMuted(_loc20_) || _loc19_)
               {
                  _loc25_ = new InterfaceEvent("onMessageMap");
                  _loc25_.text = _loc24_;
            ...
            {
               _loc14_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_USER_PID));
               _loc26_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_SMILEY_PACK_ID));
               _loc27_ = uint(param3.message.bitReadUnsignedInt(GlobalProperties.BIT_SMILEY_ID));
      ...
      public function onEndLeaveMapTeleportSameMap(param1:Event) : *
      {
         ...
         param1.currentTarget.lastSocketMessage.bitPosition = param1.currentTarget.lastSocketMessage.bitPosition - GlobalProperties.BIT_METHODE_ID;
         var _loc2_:uint = param1.currentTarget.lastSocketMessage.bitReadUnsignedInt(GlobalProperties.BIT_METHODE_ID);
      ...
      public function gotoMap(param1:ServerMap, param2:uint = 0) : *
      {
         ...
         else if(param1.serverId != serverId && mainUser)
         {
      ...
      {
         if(this._blablaland && param1 != this._blablaland)
         {
            this._blablaland.removeEventListener("onSmileyListChange",this.onSmileyListChangeEvt,false);
         }
         if(param1 && param1 != this._blablaland)
         {
