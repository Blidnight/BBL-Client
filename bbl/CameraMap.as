package bbl
{
   ...
   public class CameraMap extends MapFx
   {
      ...
      {
         var _loc1_:uint = 5 - quality.persoMoveQuality;
      ...
      {
         var _loc2_:* = undefined;
         var _loc3_:Number = GlobalProperties.getTimer() - this._lastEngineTime;
         var _loc4_:Number = _loc3_ / this.stepRate;
         var _loc5_:* = _loc4_ - this._lastEngineCount;
         _loc5_ = Math.min(_loc5_,500 / this.stepRate);
         while(_loc5_ > 0)
         {
            _loc5_--;
      ...
      public function removeAllUser() : *
      {
         ...
         var _loc4_:Array = this.userList.concat(this.tempUserList);
         this.userList.splice(0,this.userList.length);
         this.tempUserList.splice(0,this.tempUserList.length);
         while(_loc4_.length)
         {
            _loc1_ = _loc4_.shift();
            _loc2_ = new WalkerPhysicEvent("onLostUser");
            _loc2_.walker = _loc1_;
            dispatchEvent(_loc2_);
            if(_loc1_ == this.cameraTarget)
            {
               this.cameraTarget = null;
            }
            this.userRemoveListener(_loc1_);
            currentMap.onLostUser(_loc1_,0);
            _loc1_.camera = null;
            if(_loc1_.parent)
            {
               _loc1_.parent.removeChild(_loc1_);
            }
            if(_loc1_ != this._mainUser)
            {
               _loc1_.dispose();
            }
         }
      ...
      public function subRemoveUser(param1:User, param2:uint = 0) : *
      {
         ...
         var _loc4_:uint = this.userList.length;
         var _loc5_:int = 0;
         while(_loc5_ < this.userList.length)
         {
            if(this.userList[_loc5_] == param1)
            {
               _loc3_ = new WalkerPhysicEvent("onLostUser");
               _loc3_.walker = param1;
               dispatchEvent(_loc3_);
               if(this.userList[_loc5_] == this._mainUser)
               {
                  this._mainUser = null;
               }
               if(this.userList[_loc5_] == this.cameraTarget)
               {
                  this.cameraTarget = null;
               }
               currentMap.onLostUser(this.userList[_loc5_],param2);
               this.userList[_loc5_].dispose();
               this.userRemoveListener(this.userList[_loc5_]);
               this.userList[_loc5_].camera = null;
               this.userList.splice(_loc5_,1);
               break;
            }
            _loc5_++;
         }
         if(_loc4_ != this.userList.length)
         {
            dispatchEvent(new Event("onMapCountChange"));
         }
         this.CameraMapUpdateQuality();
      ...
      public function getUserByPid(param1:uint = 0) : User
      {
         ...
         if(!_loc3_)
         {
            _loc2_ = this.tempUserList.length - 1;
            while(_loc2_ >= 0)
            {
               if(this.tempUserList[_loc2_].userPid == param1)
               {
                  _loc3_ = this.tempUserList[_loc2_];
                  break;
               }
               _loc2_--;
            }
         ...
         if(!_loc3_)
         {
            _loc2_ = this.userList.length - 1;
            while(_loc2_ >= 0)
            {
               if(this.userList[_loc2_].userPid == param1)
               {
                  _loc3_ = this.userList[_loc2_];
                  break;
               }
               _loc2_--;
            }
      ...
      public function moveUserContent(param1:User, param2:String) : *
      {
         ...
         if(param1.parent)
         {
            ...
            if(_loc3_ && _loc3_ != param1.parent)
            {
      ...
      function desactive(param1:Event = null) : *
      {
         this._leftKey = false;
         this._upKey = false;
         this._shiftKey = false;
      ...
      function KeyUpEvent(param1:KeyboardEvent) : *
      {
      ...
      function KeyDnEvent(param1:KeyboardEvent) : *
      {
         this._shiftKey = param1.shiftKey;
      ...
      public function updateKeyEvent() : *
      {
         ...
         if(this._mainUser)
         {
            ...
            _loc1_ = this._leftKey && this._rightKey?0:int(int(_loc1_));
            _loc2_ = !!this._upKey?1:!!this._downKey?-1:0;
            _loc2_ = this._upKey && this._downKey?0:int(int(_loc2_));
