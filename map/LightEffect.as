package map
{
   ...
   public class LightEffect
   {
      ...
      public function redraw() : *
      {
         ...
         var _loc3_:Number = Math.pow(Math.sin(this.dayTime * Math.PI),2);
         var _loc4_:Number = _loc3_ * 0.5 + 0.5;
         var _loc5_:ColorTransform = new ColorTransform(_loc4_,_loc4_,_loc4_,1,0,0,0,0);
         if(this.temperature < 0.5)
         {
            _loc2_ = 1 - this.temperature * 2;
            _loc1_ = new ColorTransform(1,1,_loc2_ * 0.3 + 1,1,0,0,0,0);
         }
         else
         {
            _loc2_ = (this.temperature - 0.5) * 2;
            _loc1_ = new ColorTransform(1 + _loc2_ * 0.1,1,1,1,0,0,0,0);
         }
         _loc4_ = this.stormy * 0.6 * Math.pow(this.cloudDensity,2) * 1.6 * -50;
         _loc4_ = _loc4_ * (_loc3_ * 0.4 + 0.6);
         var _loc6_:ColorTransform = new ColorTransform(1,1,1,1,_loc4_,_loc4_,_loc4_,0);
         _loc5_.concat(_loc1_);
         _loc5_.concat(_loc6_);
         this.lastColorTransform = _loc5_;
         var _loc7_:* = new ColorTransform();
         var _loc8_:* = new ColorTransform();
         _loc7_.redMultiplier = 1 / _loc5_.redMultiplier;
         _loc7_.greenMultiplier = 1 / _loc5_.greenMultiplier;
         _loc7_.blueMultiplier = 1 / _loc5_.blueMultiplier;
         _loc8_.redOffset = -_loc5_.redOffset;
         _loc8_.greenOffset = -_loc5_.greenOffset;
         _loc8_.blueOffset = -_loc5_.blueOffset;
