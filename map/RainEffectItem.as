package map
{
   ...
   public class RainEffectItem extends Sprite
   {
      ...
      public function updateSnow(param1:*) : *
      {
         ...
         var _loc3_:* = Math.round(this.master.rainRateQuality * this.snowSpeed * getTimer() / 1000);
         var _loc4_:* = Math.ceil(param1 * 15);
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc2_ = Math.cos(getTimer() / 2000 + _loc5_) * 20 + _loc5_ * 30;
            this.addPass(this.master.bmdSnow,new Point(_loc5_ / _loc4_ * this.contentBmd.width + _loc2_ + 100,_loc3_ + _loc5_ / _loc4_ * this.contentBmd.height));
            _loc5_++;
         }
      }
      ...
      {
         var _loc3_:int = 0;
         var _loc4_:uint = param2.x % param1.width;
         var _loc5_:uint = param2.y % param1.height;
         var _loc6_:int = -((param1.width - _loc4_) % this.contentBmd.width);
         while(_loc6_ < this.contentBmd.width)
         {
            _loc3_ = -((param1.height - _loc5_) % this.contentBmd.height);
            while(_loc3_ < this.contentBmd.height)
            {
               this.contentBmd.copyPixels(param1,param1.rect,new Point(_loc6_,_loc3_),null,null,true);
               _loc3_ = _loc3_ + param1.height;
            }
