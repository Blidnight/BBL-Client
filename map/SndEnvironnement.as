package map
{
   ...
   public class SndEnvironnement
   {
      ...
      public function getMeteoPrct() : *
      {
         ...
         if(this.lightMode == 1)
         {
            _loc9_ = _loc9_ * (_loc1_ > this.lightThreshold?1:0);
         }
         if(this.lightMode == 2)
         {
            _loc9_ = _loc9_ * (_loc1_ < this.lightThreshold?1:0);
         ...
         {
            _loc9_ = _loc9_ * (_loc2_ > this.hotThreshold?1:0);
         }
         if(this.rainMode == 1)
         {
            _loc9_ = _loc9_ * (_loc7_ < this.rainThreshold?1:0);
         }
         if(this.snowMode == 1)
         {
            _loc9_ = _loc9_ * (_loc8_ < this.snowThreshold?1:0);
         }
         return _loc9_;
      }
      ...
      public function getSoundTransform() : SoundTransform
      {
         ...
         var _loc3_:* = new SoundTransform();
         if(this.mode == 0)
         {
            _loc3_.volume = this.volume * this.generalVolume;
         }
         else
         {
            _loc1_ = this.volume * this.generalVolume;
            if(this.meteoOnVolume)
            {
               _loc2_ = this.getMeteoPrct();
               _loc1_ = _loc1_ * _loc2_;
            }
            _loc3_.volume = _loc1_;
         }
         return _loc3_;
      }
      ...
      public function syncTimer(param1:Event) : *
      {
         var _loc2_:* = null;
         var _loc3_:* = null;
         var _loc4_:Srandom = new Srandom();
         _loc4_.seed = param1.currentTarget.syncTime + this.seed;
         var _loc5_:Number = _loc4_.generate(3);
         var _loc6_:* = this.getMeteoPrct();
         if(this.meteoOnRate)
         {
         ...
         if(this.mode == 1)
         {
            if(_loc5_ < 1 - this.rate && this.channelList.length)
            {
               this.disposeAllChannel();
            }
            else if(_loc5_ > 1 - this.rate && !this.channelList.length)
            {
               _loc2_ = new this.soundClass();
               _loc3_ = _loc2_.play(0,99999,this.getSoundTransform());
               if(_loc3_)
               {
                  this.channelList.push(_loc3_);
               }
         ...
         {
            if(_loc5_ > 1 - this.rate)
            {
               _loc2_ = new this.soundClass();
               _loc3_ = _loc2_.play(0,0,this.getSoundTransform());
               if(_loc3_)
               {
                  _loc3_.addEventListener("soundComplete",this.eventEvent,false,0,true);
                  this.channelList.push(_loc3_);
               }
            }
         }
