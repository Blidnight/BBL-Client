package map
{
   ...
   public class MeteoGenerator
   {
      ...
      public function getSeason(param1:Number) : *
      {
         ...
         var _loc7_:* = Number(param1 / this.seasonCicleDelay % 1);
         if(this.meteoId == 1)
         {
            _loc2_ = 3;
            _loc3_ = 9;
            _loc4_ = 0.25;
            _loc5_ = 0.5 + _loc4_ * _loc7_ * 2 - _loc4_;
            _loc6_ = Math.max(Math.min((this.positionX - _loc2_) / (_loc3_ - _loc2_),1),0);
            _loc7_ = Number(_loc7_ * (1 - _loc6_) + _loc5_ * _loc6_);
         }
