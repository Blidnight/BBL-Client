package map
{
   ...
   public class MapMeteo extends MapEnvironnementBuilder
   {
      ...
      public function updateInterval(param1:Event = null) : *
      {
         ...
         var _loc4_:Number = Math.abs(this.season.getSpeedAt(_loc3_));
         var _loc5_:Number = Math.abs(this.dayTime.getSpeedAt(_loc3_));
         var _loc6_:Number = Math.abs(this.cloudDensity.getSpeedAt(_loc3_));
         var _loc7_:Number = Math.abs(this.fogDensity.getSpeedAt(_loc3_));
         var _loc8_:Number = Math.abs(this.stormy.getSpeedAt(_loc3_));
         var _loc9_:Number = Math.abs(this.temperature.getSpeedAt(_loc3_));
         var _loc10_:Number = Math.abs(this.humidity.getSpeedAt(_loc3_));
         this.fogSteper.rate = _loc7_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc7_ * 30) / 8) * 8,4),512)));
         this.astroSteper.rate = _loc5_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc5_ * 100)),2),128)));
         this.snowEffectSteper.rate = _loc9_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc9_ * 20) / 4) * 4,4),512)));
         this.daytimeEffectSteper.rate = _loc5_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc5_ * 20) / 4) * 4,4),512)));
         this.seasonEffectSteper.rate = _loc4_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc4_ * 20) / 4) * 4,4),512)));
         _loc2_ = Math.max(Math.max(_loc6_,_loc9_),_loc10_);
         this.rainEffectSteper.rate = _loc2_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc2_ * 5) / 4) * 4,16),512)));
         _loc2_ = Math.max(Math.max(_loc6_,_loc8_),_loc5_ * 4);
         this.cloudSteper.rate = Math.min(Math.max(Math.round(1 / (_loc2_ * 10) / 8) * 8,4),128);
         this.skySteper.rate = _loc5_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc5_ * 30) / 4) * 4,4),512)));
         _loc2_ = Math.max(Math.max(Math.max(_loc5_ * 2,_loc9_ / 2),_loc8_),_loc6_);
         this.lightEffectSteper.rate = _loc2_ == 0?Number(Number(0)):Number(Number(Math.min(Math.max(Math.round(1 / (_loc2_ * 8) / 4) * 4,8),512)));
      }
